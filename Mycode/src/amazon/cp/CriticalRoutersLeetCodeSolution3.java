...
public class CriticalRoutersLeetCodeSolution3 {
	...
	public static List<Integer> criticalConnections(int n, List<List<Integer>> connections) {
	...
	private static void dfs(int node, int parent, int time) {
		...
		for (Integer n : adj.get(node)) {
			...
			if (disc[node] < low[n]) { // neighbour is visited after the current node (no earlier path found)
				/*
				 * List<Integer> x = new ArrayList<Integer>() { private static final long
				 * serialVersionUID = 1L; { add(n); add(node); } };
				 */
				ans.add(node);
			}
		}
	...
	public static void main(String[] args) {
		...
		x.add(8);
		conn.add(x);
		List<Integer> r = criticalConnections(9, conn);
		for (Integer c : r) {
